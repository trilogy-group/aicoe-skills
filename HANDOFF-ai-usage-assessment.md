...
## Secrets — read before committing
- **Submit token:** `aicoe-sub-Tq8vR2mK6pXw` — appears in `SKILL.md` step 9 (two spots). It is **low-sensitivity** (only allows appending `.md` files to that one Drive folder).
- It **must equal** `SUBMIT_TOKEN` in the **deployed** Apps Script (the `relay/assessment-relay.gs` here is a placeholder on purpose — do not put the real token in it).
- Because `SKILL.md` carries the token, **keep this repo private.** If it must ever go public: replace the token in `SKILL.md` with a placeholder, re-architect (treat the relay as token-less behind the obscure URL, or inject the token at install/build), and rotate the token in the deployed script.
...
- [ ] Deployed Apps Script `SUBMIT_TOKEN` == `aicoe-sub-Tq8vR2mK6pXw`, redeployed as a **New version** of the existing deployment (same `/exec` URL).
- [ ] Repo is **private**.
