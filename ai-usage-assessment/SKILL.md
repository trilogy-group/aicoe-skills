...
9. **Auto-submit to the AI CoE.** Send the report to the CoE collection relay so it lands centrally
   ...
   - **Submit token:** `aicoe-sub-Tq8vR2mK6pXw`
   - Write the payload, then POST (this handles quoting/newlines safely):
     ```bash
     python3 -c "import json; open('payload.json','w').write(json.dumps({'token':'aicoe-sub-Tq8vR2mK6pXw','person':'<First Last>','filename':'ai-usage_<firstname-lastname>.md','report':open('ai-usage_<firstname-lastname>.md').read()}))"
